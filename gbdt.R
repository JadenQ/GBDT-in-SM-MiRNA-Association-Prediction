...
#try k-means
########
  negativeSampleIndices<-sample(allIndices[-knownMSAIndices], size = 2652, replace = F)
  # negativeSampleIndices<-matrix(rep(0,90000),nrow=9000,ncol=10)
...
##############
  ...
  # positiveAndNegativeIndices<-matrix(rep(0,(length(knownMSAIndices)+9000)*10),nrow=10,ncol=9663)
  # for(k in 1:10){
  ...
  trainingAndTestingData <- BuildTrainingAndTestingData(MSA, similaritiesOfMiRNA, similaritiesOfSM, m, s, knownMSAIndices, 
                                                        ...
                                                        localLoocvTestingIndices,localLoocvTestingIndices2)
 # train=data.frame(trainingAndTestingData$loocvTrainingFeatureVectors)
  #train=trainingAndTestingData$loocvTrainingFeatureVectors
  
  #X_train=data.frame(trainingAndTestingData$loocvTrainingFeatureVectors[, -1])
  ...
  train=data.frame(cbind(X_train,Y_train))
    # gbm1<-gbm(as.factor(Y_trian)~.,         # formula
    # data=train,                   # dataset
  ...
  # gbm1<-gbm(Y_train~noOfObervationsOfMiRNA+aveOfSimilaritiesOfMiRNA+hist1MiRNA 
  #                                +hist2MiRNA+hist3MiRNA+hist4MiRNA 
  #                                +hist5MiRNA+noOfObervationsOfSM+aveOfSimilaritiesOfSM+hist1SM
  #                             +hist2SM+hist3SM+hist4SM
  #                             +hist5SM+numberOfNeighborsMiRNA+knn1SimilarityMiRNA+knn2SimilarityMiRNA+knn3SimilarityMiRNA 
  #                                +knn4SimilarityMiRNA+knn5SimilarityMiRNA+knn6SimilarityMiRNA+knn7SimilarityMiRNA
  #                                +knn8SimilarityMiRNA+knn9SimilarityMiRNA+knn10SimilarityMiRNA+aveNoObsMiRNA
  #                                +aveOfAveSimilarityMiRNA+aveHist1MiRNA+aveHist2MiRNA+aveHist3MiRNA+aveHist4MiRNA
  #                                +aveHist5MiRNA+weightedAveNoObsMiRNA+weightedAveOfAveSimilarityMiRNA+weightedAveHist1MiRNA
  #                                +weightedAveHist2MiRNA+weightedAveHist3MiRNA+weightedAveHist4MiRNA+weightedAveHist5MiRNA
  #                                +betweennessCentralityMiRNA+closenessCentralityMiRNA+eigenVectorCentralityMiRNA+pageRankMiRNA+numberOfNeighborsSM+knn1SimilaritySM+knn2SimilaritySM+knn3SimilaritySM
  #                             +knn4SimilaritySM+knn5SimilaritySM+knn6SimilaritySM+knn7SimilaritySM
  #                             +knn8SimilaritySM+knn9SimilaritySM+knn10SimilaritySM+aveNoObsSM
  #                             +aveOfAveSimilaritySM+aveHist1SM+aveHist2SM+aveHist3SM+aveHist4SM
  #                             +aveHist5SM+weightedAveNoObsSM+weightedAveOfAveSimilaritySM+weightedAveHist1SM
  #                             +weightedAveHist2SM+weightedAveHist3SM+weightedAveHist4SM+weightedAveHist5SM
  #                             +betweennessCentralitySM+closenessCentralitySM+eigenVectorCentralitySM+pageRankSM+latentVectors1MiRNA+latentVectors2MiRNA+latentVectors3MiRNA+latentVectors4MiRNA+latentVectors5MiRNA
  #                               +latentVectors1SM+latentVectors2SM+latentVectors3SM+latentVectors4SM+latentVectors5SM
  #                                +numberOfSMNeighborAssociations
  #                                +numberOfMiRNANeighborAssociations+betweennessCentralityMiRNAInMSA+closenessCentralityMiRNAInMSA+eigenVectorCentralityMiRNAInMSA
  #                                +pageRankMiRNAInMSA+betweennessCentralitySMInMSA+closenessCentralitySMInMSA+eigenVectorCentralitySMInMSA
  #                                +pageRankSMInMSA,         # formula
  #   data=train,                   # dataset
  #   distribution="gaussian",     # see the help for other choices
  ...
  #   n.cores=1)                   # use only a single core (detecting #cores is
  #                                # error-prone, so avoided here)
#  gbm1<-gbm.fit(x=X_train,y=Y_train,     # formula
#     distribution="bernoulli",     # see the help for other choices
#     n.trees=20,                # number of trees
#     shrinkage=0.008,              # shrinkage or learning rate,
#                                  # 0.001 to 0.1 usually work
#     interaction.depth=1,         # 1: additive model, 2: two-way interactions, etc.
#     bag.fraction = 0.5,          # subsampling fraction, 0.5 is probably best
#     train.fraction = 0.5,        # fraction of data for training,
...
gbm1=train(X_train, Y_train, method = "gbm", preProcess = NULL, 
  ...
  #                         label = trainingAndTestingData$loocvTrainingFeatureVectors[, 1], params = parameters, nthread = 2, nrounds = 3, 
...
#   }
#   #get 10 different results for old gbm
#     # allpredictedWeightsGlobal <- rbind(allpredictedWeightsGlobal,predict(object=gbm1, newdata=data.frame(trainingAndTestingData$globalLoocvTestingFeatureVectors),n.trees=1500,type="response"))
#     # allpredictedWeightsLocal <- rbind(allpredictedWeightsLocal,predict(object=gbm1, newdata=data.frame(trainingAndTestingData$localLoocvTestingFeatureVectors),n.trees=1500,type="response"))
#     # allpredictedWeightsLocal2 <- rbind(allpredictedWeightsLocal2,predict(object=gbm1, newdata=data.frame(trainingAndTestingData$localLoocvTestingFeatureVectors2),n.trees=1500,type="response"))
#   #new caret gbm
#      allpredictedWeightsGlobal <- rbind(allpredictedWeightsGlobal,predict(gbm1, data.frame(trainingAndTestingData$globalLoocvTestingFeatureVectors)))
#     allpredictedWeightsLocal <- rbind(allpredictedWeightsLocal,predict(gbm1, data.frame(trainingAndTestingData$localLoocvTestingFeatureVectors)))
#     allpredictedWeightsLocal2 <- rbind(allpredictedWeightsLocal2,predict(gbm1, data.frame(trainingAndTestingData$localLoocvTestingFeatureVectors2)))
...
# predictedWeightsLocal2<-colMeans(allpredictedWeightsLocal2)
 ...
 predictedWeightsGlobal <- predict(gbm1, data.frame(trainingAndTestingData$globalLoocvTestingFeatureVectors))
